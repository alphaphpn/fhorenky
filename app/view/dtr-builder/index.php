<?php
	...
	if (empty($_SESSION["uid"]) || empty($_SESSION["uname"]) || empty($_SESSION["ulevel"]) || empty($_SESSION["uposition"]) || empty($_SESSION["ustat"]) || empty($_SESSION["verified"])) {
		...
		// Head Officer
		echo '<script>alert("Access Deinied!");window.open("../../routes/login","_self");</script>';
															...
															echo '<tr>';
																echo '<td colspan="14">';
																	echo '<div class="alert alert-danger alert-dismissible fade show">';
																		...
																		echo 'Would you like to Generate their DTR? <a href="../../routes/dtr-builder/?createnew=1&filteryear='.$yearnox.'&filtermonth='.$monthnox.'&biolocation='.$biolocation2.'&timelogtype='.$timelogtypex.'" target="_blank">Yes</a>';
																	echo '</div>';
																echo '</td>';
															echo '</tr>';
														}
